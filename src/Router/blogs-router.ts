...
BlogsRouter.get('/', async (req: Request, res: Response) => {
    const findBlogs: BlogsType[] = await BlogsRepository.findBlog(req.query.name?.toString())
    res.status(200).send(findBlogs)
})
BlogsRouter.get('/:id',
    async (req: Request, res: Response) => {
        let video: BlogsType | null = await BlogsRepository.findBlogByID(req.params.id)
        if (video) {
...
BlogsRouter.put('/:id', mwBasicAuth, nameValidation, youtubeUrlValidation, inputValidationMiddleware, async (req: Request, res: Response) => {
    const isUpdate: boolean = await BlogsRepository.updateBlog(req.params.id, req.body.name, req.body.youtubeUrl)
    if (isUpdate) {
